...
function update_package() {
  ...
  case ${package_name} in
    ...
    "sqlite3")
      ...
      ;;
    "libogg")
      package_version_latest=$(wget -q -O- 'https://www.xiph.org/downloads/' | sed -n 's,.*libogg-\([0-9][^>]*\)\.tar.*,\1,p' | head -1)
      ;;
    "libvorbis")
      package_version_latest=$(wget -q -O- 'https://www.xiph.org/downloads/' | sed -n 's,.*libvorbis-\([0-9][^>]*\)\.tar.*,\1,p' | head -1)
      ;;
    "flac")
    ...
    "fdk_aac")
      ...
      ;;
    "libbs2b")
      package_version_latest=$(wget -q -O- 'https://sourceforge.net/projects/bs2b/files/libbs2b/' | sed -n 's,.*<a href="/projects/bs2b/files/libbs2b/\([0-9][^"]*\)/".*,\1,p' | head -1)
      ;;
    "chromaprint")
    ...
    "harfbuzz")
      ...
      ;;
    "protobuf")
      package_version_latest=$(wget -q -O- 'https://github.com/protocolbuffers/protobuf/tags' | sed -n 's#.*releases/tag/\([^"]*\).*#\1#p' | sed 's/^v//g' | sort -V | head -1)
      ;;
    "qt")
